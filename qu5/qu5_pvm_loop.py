...
class Qu4(nn.Module):
    ...
    def __init__(self):
        ...
        self.params = nn.Parameter(tc.randn(305,dtype=tc.float64),requires_grad=True)
    ...
    def generate_state(self,a):
        ...
        psi01 = tc.cat((a[0],tc.zeros(5,device=self.params.device),a[1],tc.zeros(5,device=self.params.device),a[2], tc.zeros(5,device=self.params.device), a[3] , tc.zeros(5,device=self.params.device),a[4]))
        psi23 = psi01
        psi45 = psi01
    ...
    def generate_POVM(self,x):
        ...
        params = x
        A = tc.zeros((25, 25), dtype=tc.float64,device=self.params.device)
        D_i = tc.ones(r,dtype=tc.float64,device=self.params.device)

        Z = tc.zeros(25-r,dtype=tc.float64,device=self.params.device)
        ...
        idx = tc.triu_indices(25, 25, offset=1)  # indices for the upper triangular part excluding the diagonal
        A[idx[0], idx[1]] = params
        A = A - A.T  # to make A skew-symmetric

        # generate an orthogonal matrix O using the skew-symmetric matrix
        M = tc.eye(25,device=self.params.device) + A
        U = tc.mm(M, tc.inverse(tc.eye(25,device=self.params.device) - A))
    ...
    def loss_KL(self):
        ...
        m1 = self.generate_POVM(self.params[5:305])
        m1_ = tc.eye(25,device=self.params.device) - m1
        m2 = m1
        m2_ = tc.eye(25,device=self.params.device) - m2
        m3 = m1
        m3_ = tc.eye(25,device=self.params.device) - m3
...
def generate_swapmatrix():
    ...
    s01 = tc.kron(s,tc.eye(5**4,device = 'cuda'))

    s12_ = tc.kron(tc.eye(5,device = 'cuda'),s)
    s12 = tc.kron(s12_,tc.eye(5**3,device = 'cuda'))

    s23_ = tc.kron(tc.eye(5**2,device = 'cuda'),s)
    s23 = tc.kron(s23_,tc.eye(5**2,device = 'cuda'))

    s34_ = tc.kron(tc.eye(5**3,device = 'cuda'),s)
    s34 = tc.kron(s34_,tc.eye(5,device = 'cuda'))

    s45 = tc.kron(tc.eye(5**4,device = 'cuda'),s)
...
for i in range(12):
    r=i+1
    print('r=:',r)
    optimizer = tc.optim.Adam(model.parameters(), lr=0.001)  # Use Adam for optimization.

    model.optimize(optimizer)

    model.draw()
    psi01 = model.show_state(model.params[0:5])
    print(psi01)

    #print(model.params)

    state = model.generate_state(model.params[0:5])

    state_swaped = model.swap_cyclic(state)
    rou = model.density_matrix(state_swaped)

    m1 = model.generate_POVM(model.params[5:305])
    m1_ = tc.eye(25,device=model.params.device) - m1
    m2 = m1
    m2_ = tc.eye(25,device=model.params.device) - m2
    m3 = m1
    m3_ = tc.eye(25,device=model.params.device) - m3
